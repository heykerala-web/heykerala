...
export default function PlaceForm({ initialData, isEdit = false }: PlaceFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEdit) {
                await api.put(`/admin/places/${initialData._id}`, payload);
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl bg-white p-6 rounded-xl border shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    <Input id="slug" name="slug" value={formData.slug} onChange={handleChange} required placeholder="munnar-hills" />
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    ...
                    <label className="flex flex-col items-center justify-center border-2 border-dashed rounded-lg aspect-square cursor-pointer hover:bg-gray-50 transition-colors">
                        {uploading ? (
                ...
                <p className="text-xs text-gray-500">First image will be the main cover.</p>
            </div>

            <Button type="submit" disabled={loading} className="w-full bg-emerald-600 hover:bg-emerald-700">
                {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                {isEdit ? "Update Place" : "Create Place"}
            </Button>
        </form>
    );
}
