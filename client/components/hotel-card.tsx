...
export function HotelCard({
  ...
  return (
    <Link href={`/stay/${id}`} className="block h-full group">
      <article className="rounded-[2rem] bg-card border border-border shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden flex flex-col h-full hover:-translate-y-2">
        <div className="relative h-52 overflow-hidden">
          <img src={getFullImageUrl(image, name, "hotel", undefined, updatedAt)} alt={name} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
          <button
            aria-label={`${bookmarked ? "Remove from" : "Add to"} bookmarks`}
            onClick={handleSave}
            disabled={saving}
            className="absolute right-4 top-4 rounded-full bg-white/95 backdrop-blur-md p-2.5 hover:bg-white transition-all duration-300 shadow-sm border border-black/5 active:scale-90 disabled:opacity-60 z-20"
          >
